...
{
public:
	CVertexLightThread(u32 ID) : CThread(ID) { thMessages = TRUE; }
	virtual void Execute()
	{
		CDB::COLLIDER DB;
		DB.ray_options(0);
		...
		for (;; counter++)
		{
			...
			if (bVertexLight)
			{
				base_color_c vC, old;
				V->C._get(old);
				LightPoint(&DB, RCAST_Model, vC, V->P, V->N, pBuild->L_static,
					(b_norgb ? LP_dont_rgb : 0) | (b_nosun ? LP_dont_sun : 0) | LP_dont_hemi, 0);
				vC._tmp_ = v_trans;
				vC.mul(.5f);
