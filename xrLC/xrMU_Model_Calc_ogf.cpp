...
void xrMU_Model::calc_ogf()
{
	...
	for (xrMU_Model::v_subdivs_it it = m_subdivs.begin(); it != m_subdivs.end(); ++it)
	{
		...
		try
		{
			...
			{
				clMsg("* ERROR: MU2OGF, model %s, *faces*", *m_name);
			}
		}
		catch (...)
		{
		...
		{
			clMsg("* ERROR: MU2OGF, [optimize], model %s", *m_name);
		}
		try
		{
			pOGF->CalcBounds();
		...
		{
			clMsg("* ERROR: MU2OGF, [bounds], model %s", *m_name);
		}
		try
		{
			pOGF->CalculateTB();
		...
		{
			clMsg("* ERROR: MU2OGF, [calc_tb], model %s", *m_name);
		}
		try
		{
		...
		{
			clMsg("* ERROR: MU2OGF, [progressive], model %s", *m_name);
		}
		try
		{
			pOGF->Stripify();
		...
		{
			clMsg("* ERROR: MU2OGF, [stripify], model %s", *m_name);
		}
