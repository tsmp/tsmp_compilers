...
u16 OGF::x_BuildVertex(x_vertex &V1)
{
	for (itXV it = x_vertices.begin(); it != x_vertices.end(); ++it)
		if (it->similar(this, V1))
			return u16(it - x_vertices.begin());
		...
		else
		{
			...
			for (u32 vid = 0; vid < 3; vid++)
			{
				...
				for (u32 sid = 0; sid < dest.size(); sid++)
				{
					...
					{
						bConnected =
							TRUE; // this face shares at least one vertex with already selected faces
						break;
					}
				}
				if (bConnected)
...
void OGF::Optimize()
{
	...
	*/
	...
	vmarker.assign(vertices.size(), false);
	xr_vector<bool> fmarker;
	fmarker.assign(faces.size(), false);
...
void OGF::CalculateTB()
{
	...
	if (!mender.Munge(input,		   // input attributes
			...
			NVMeshMender::
				DontFixCylindricalTexGen, // handle cylindrically mapped textures via vertex duplication
...
void OGF::MakeProgressive(float metric_limit)
{
	...
	if (progressive_test() && x_vertices.size() && x_faces.size())
	{
		...
		for (u32 v_idx = 0; v_idx < x_vertices.size(); v_idx++)
			VIPM_AppendVertex(x_vertices[v_idx].P, zero);
		for (u32 f_idx = 0; f_idx < x_faces.size(); f_idx++)
			VIPM_AppendFace(x_faces[f_idx].v[0], x_faces[f_idx].v[1], x_faces[f_idx].v[2]);
