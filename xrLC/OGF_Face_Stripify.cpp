...
void OGF::Stripify()
{
		...
		*/
	{
		// alternative stripification - faces
		{
			...
			HRESULT rhr = D3DXOptimizeFaces(&x_faces.front(), x_faces.size(), x_vertices.size(),
				FALSE, remap);
			R_CHK(rhr);
			vecOGF_F _source = x_faces;
			for (u32 it = 0; it < _source.size(); it++)
				x_faces[it] = _source[remap[it]];
			xr_free(remap);
		}
		...
		// alternative stripification - vertices
		{
			...
			HRESULT rhr = D3DXOptimizeVertices(&x_faces.front(), x_faces.size(), x_vertices.size(),
				FALSE, remap);
			R_CHK(rhr);
			vec_XV _source = x_vertices;
			for (u32 it = 0; it < _source.size(); it++)
				x_vertices[remap[it]] = _source[it];
			for (u32 it = 0; it < x_faces.size(); it++)
				for (u32 j = 0; j < 3; j++)
					x_faces[it].v[j] = (u16)remap[x_faces[it].v[j]];
			xr_free(remap);
		}
	}
...
void OGF::DumpFaces()
{
	...
	for (u32 i = 0; i < faces.size(); i++)
		clMsg("face #%4d: %4d %4d %4d", i, int(faces[i].v[0]), int(faces[i].v[1]),
			int(faces[i].v[2]));
	clMsg("fast:");
	for (u32 i = 0; i < x_faces.size(); i++)
		clMsg("face #%4d: %4d %4d %4d", i, int(x_faces[i].v[0]), int(x_faces[i].v[1]),
			int(x_faces[i].v[2]));
}
