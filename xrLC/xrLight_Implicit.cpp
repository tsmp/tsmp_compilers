				...
				for (u32 J = 0; J < Jcount; J++)
				{
					...
					for (vecFaceIt it = space.begin(); it != space.end(); ++it)
					{
						...
						if (tc.isInside(P, B))
						{
							...
							wN.normalize();
							LightPoint(&DB, RCAST_Model, C, wP, wN, pBuild->L_static,
								(b_norgb ? LP_dont_rgb : 0) | (b_nosun ? LP_dont_sun : 0), F);
							Fcount++;
						}
					}
				}
...
void CBuild::ImplicitLighting()
{
	...
	for (Implicit_it imp = calculator.begin(); imp != calculator.end(); ++imp)
	{
		...
		for (u32 itr = 0; itr < count; itr++)
			implicit_task_pool.push_back(itr);
