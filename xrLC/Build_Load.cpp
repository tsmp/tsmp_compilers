...
void CBuild::Load(const b_params &Params, const IReader &_in_FS, HWND Window)
{
	...
	Status("Processing textures...");
	{
		...
		for (u32 t = 0; t < tex_count; t++)
		{
			...
			{
				string_path th_name;
				FS.update_path(th_name, "$textures$",
		...
		if (is_thm_missing)
			MessageBox(Window, "Some of required thm's are missing. See log for details.", "Error!",
				MB_OK | MB_ICONERROR);
		if (is_tga_missing)
			MessageBox(Window, "Some of required tga textures are missing. See log for details.",
