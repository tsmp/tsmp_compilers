...
void CDeflector::L_Direct_Edge(CDB::COLLIDER *DB, base_lighting *LightsSelected, Fvector2 &p1,
	Fvector2 &p2, Fvector &v1, Fvector &v2, Fvector &N, float texel_size, Face *skip)
{
	Fvector vdir;
	vdir.sub(v2, v1);
	...
	for (int I = 0; I <= steps; I++)
	{
		...
		Fvector P;
		P.mad(v1, vdir, time);
		LightPoint(DB, RCAST_Model, C, P, N, *LightsSelected,
			(b_norgb ? LP_dont_rgb : 0) | (b_nosun ? LP_dont_sun : 0) | LP_DEFAULT, skip); //.
				...
				for (u32 J = 0; J < Jcount; J++)
				{
					...
					for (UVtri **it = &*space.begin(); it != &*space.end(); it++)
					{
						if ((*it)->isInside(P, B))
						{
							...
							try
							{
								LightPoint(DB, RCAST_Model, C, wP, wN, *LightsSelected,
									(b_norgb ? LP_dont_rgb : 0) | (b_nosun ? LP_dont_sun : 0) |
										LP_UseFaceDisable,
									F); //.
								Fcount += 1;
							}
