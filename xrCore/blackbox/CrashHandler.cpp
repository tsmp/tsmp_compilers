...
BOOL __stdcall AddCrashHandlerLimitModule(HMODULE hMod)
{
	...
	HMODULE *phTemp = (HMODULE *)HeapAlloc(GetProcessHeap(),
		HEAP_ZERO_MEMORY | HEAP_GENERATE_EXCEPTIONS, (sizeof(HMODULE) * (g_uiModCount + 1)));
	ASSERT(NULL != phTemp);
	if (NULL == phTemp)
	{
...
LONG __stdcall CrashHandlerExceptionFilter(EXCEPTION_POINTERS *pExPtrs)
{
	...
	__try
	{
		...
		if (NULL != g_pfnCallBack)
		{
			...
			{
				HINSTANCE hBaseAddr =
					(HINSTANCE)SymGetModuleBase((HANDLE)(size_t)GetCurrentProcessId(),
...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS *pExPtrs)
{
	...
	__try
	{
		...
		if (NULL != dwTemp)
		{
			iCurr += wsprintf(g_szBuff + iCurr, _T ( "%u" ), dwTemp);
		}
		else
		{
		...
		dwTemp = SymGetModuleBase((HANDLE)(size_t)GetCurrentProcessId(),
		...
		{
			iCurr += BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)(size_t)dwTemp,
...
#else
		iCurr += wsprintf(g_szBuff + iCurr, _T ( " at %04X:%08p" ), pExPtrs->ContextRecord->SegCs,
			pExPtrs->ExceptionRecord->ExceptionAddress);
#endif
		...
		DWORD dwDisp;
		if (TRUE == SymGetSymFromAddr((HANDLE)(size_t)GetCurrentProcessId(),
#ifdef _WIN64
						(DWORD64)pExPtrs->ExceptionRecord->
#else
						(DWORD)pExPtrs->ExceptionRecord->
						...
						pSym))
		{
			...
			else
			{
				...
				{
					iCurr +=
						wsprintf(g_szBuff + iCurr, _T ( "%s()+%04d byte(s)" ), pSym->Name, dwDisp);
				}
				else
				{
		...
		if (TRUE == InternalSymGetLineFromAddr((HANDLE)(size_t)GetCurrentProcessId(),
#ifdef _WIN64
						(DWORD64)pExPtrs
							->
#else
						...
						&dwDisp, &g_stLine))
		{
			...
			else
			{
				...
				{
					iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s, line %04d"), g_stLine.FileName,
						g_stLine.LineNumber);
				}
			}
		}
...
LPCTSTR __stdcall GetFirstStackTraceString(DWORD dwOpts, EXCEPTION_POINTERS *pExPtrs)
{
	...
	//                                  (DWORD)pExPtrs->ContextRecord->IntRa;
	//    g_stFrame.AddrReturn.Mode     = AddrModeFlat ;
	g_stFrame.AddrStack.Offset = (DWORD)pExPtrs->ContextRecord->Rsp;
	g_stFrame.AddrStack.Mode = AddrModeFlat;
	g_stFrame.AddrFrame.Offset = (DWORD)pExPtrs->ContextRecord->Rbp;
	g_stFrame.AddrFrame.Mode = AddrModeFlat;
#endif
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
		...
		BOOL bSWRet = StackWalk(CH_MACHINE, (HANDLE)(size_t)GetCurrentProcessId(),
			GetCurrentThread(), &g_stFrame, pExPtrs->ContextRecord,
		...
		dwModBase =
			SymGetModuleBase((HANDLE)(size_t)GetCurrentProcessId(), g_stFrame.AddrPC.Offset);
		if (0 == dwModBase)
		{
			szRet = NULL;
			return (szRet);
		...
		if (GSTSO_MODULE == (dwOpts & GSTSO_MODULE))
		{
			...
			ASSERT(iCurr < (BUFF_SIZE - MAX_PATH));
			iCurr += BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)(size_t)dwModBase,
				g_szBuff + iCurr, BUFF_SIZE - iCurr);
		}
		...
		if (GSTSO_SYMBOL == (dwOpts & GSTSO_SYMBOL))
		{
							...
							pSym))
			{
				...
				else
				{
					if (dwDisp > 0)
					{
						iCurr += wsprintf(g_szBuff + iCurr, _T( "%s(%u)"), pSym->Name, dwDisp);
					}
					else
					{
						iCurr += wsprintf(g_szBuff + iCurr, _T ( "%s" ), pSym->Name);
...
LPCTSTR __stdcall GetRegisterString(EXCEPTION_POINTERS *pExPtrs)
{
	...
	wsprintf(g_szBuff,
		...
		pExPtrs->ContextRecord->EFlags, pExPtrs->ContextRecord->SegCs,
		pExPtrs->ContextRecord->SegDs, pExPtrs->ContextRecord->SegSs, pExPtrs->ContextRecord->SegEs,
