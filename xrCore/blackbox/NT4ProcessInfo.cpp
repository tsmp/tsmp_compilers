...
typedef BOOL(
	WINAPI *ENUMPROCESSMODULES)(HANDLE hProcess, HMODULE *lphModule, DWORD cb, LPDWORD lpcbNeeded);

typedef DWORD(
	WINAPI *GETMODULEBASENAME)(HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName, DWORD nSize);

typedef DWORD(
	WINAPI *GETMODULEFILENAMEEX)(HANDLE hProcess, HMODULE hModule, LPTSTR lpFilename, DWORD nSize);
...
----------------------------------------------------------------------*/
BOOL __stdcall NT4GetLoadedModules(DWORD dwPID, UINT uiCount, HMODULE *paModArray,
...
DWORD __stdcall NTGetModuleBaseName(HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName,
	DWORD nSize)
{
	// Initialize PSAPI.DLL, if needed.
	if (FALSE == InitPSAPI())
...
DWORD __stdcall NTGetModuleFileNameEx(DWORD /*dwPID*/, HANDLE hProcess, HMODULE hModule,
	LPTSTR szFilename, DWORD nSize)
{
	// Initialize PSAPI.DLL, if needed.
	if (FALSE == InitPSAPI())
	{
