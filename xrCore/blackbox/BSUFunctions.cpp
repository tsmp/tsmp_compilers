...
static DWORD __stdcall Win95GetModuleBaseName(HANDLE hProcess, HMODULE hModule, LPSTR lpBaseName,
	DWORD nSize);
...
DWORD __stdcall BSUGetModuleBaseName(HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName,
	DWORD nSize)
{
	if (TRUE == IsNT())
	{
...
DWORD __stdcall BSUSymInitialize(DWORD dwPID, HANDLE hProcess, PSTR UserSearchPath,
	BOOL fInvadeProcess)
{
	...
	else
	{
		...
		for (UINT uiCurr = 0; uiCurr < dwCount; uiCurr++)
		{
			...
			// ends.
			HANDLE hFile =
				CreateFile(szModName, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_EXISTING, 0, 0);
...
DWORD __stdcall BSUGetModuleFileNameEx(DWORD dwPID, HANDLE hProcess, HMODULE hModule,
