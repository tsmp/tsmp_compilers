...
bool EFS_Utils::GetOpenName(LPCSTR initial, xr_string &buffer, bool bMulti, LPCSTR offset,
	int start_flt_ext)
{
	string_path buf;
...
BOOL EFS_Utils::CheckLocking(LPCSTR fname, bool bOnlySelf, bool bMsg) //, shared_str* owner)
{
	...
	if (FS.exist(fn))
	{
		HANDLE handle = CreateFile(fn, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0,
