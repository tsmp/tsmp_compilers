...
{
	BOOL result = StackWalk(MACHINE_TYPE, GetCurrentProcess(), GetCurrentThread(), stackFrame,
		threadCtx, nullptr, SymFunctionTableAccess, SymGetModuleBase, nullptr);

	if (!result || !stackFrame->AddrPC.Offset)
		return false;

	frameStr.clear();
	HINSTANCE hModule = reinterpret_cast<HINSTANCE>(
		SymGetModuleBase(GetCurrentProcess(), stackFrame->AddrPC.Offset));
	...
	sprintf_s(formatBuff, _countof(formatBuff), " at %p",
	...
	result = SymGetSymFromAddr(GetCurrentProcess(), stackFrame->AddrPC.Offset, &dwFunctionOffset,
		functionInfo);

	if (result)
	{
		if (dwFunctionOffset)
			sprintf_s(formatBuff, _countof(formatBuff), " %s() + %Iu byte(s)", functionInfo->Name,
				dwFunctionOffset);
		else
	...
	result = SymGetLineFromAddr(GetCurrentProcess(), stackFrame->AddrPC.Offset, &dwLineOffset,
		&sourceInfo);
	...
	if (result)
	{
		if (dwLineOffset)
		{
			sprintf_s(formatBuff, _countof(formatBuff), " in %s line %u + %u byte(s)",
				sourceInfo.FileName, sourceInfo.LineNumber, dwLineOffset);
		}
		else
			sprintf_s(formatBuff, _countof(formatBuff), " in %s line %u", sourceInfo.FileName,
...
void gather_info(const char *expression, const char *description, const char *argument0,
	const char *argument1, const char *file, int line, const char *function, LPSTR assertion_info)
{
...
void xrDebug::backend(const char *expression, const char *description, const char *argument0,
	const char *argument1, const char *file, int line, const char *function, bool &ignore_always)
{
...
LPCSTR xrDebug::error2string(long code)
{
	...
	if (0==result) 
	{*/
	FormatMessage(FORMAT_MESSAGE_FROM_SYSTEM, 0, code, 0, desc_storage, sizeof(desc_storage) - 1,
		0);
	result = desc_storage;
	//	}
	return result;
