...
IC float CLevelGraph::square(float a1, float b1, float fAlpha) const
{
	...
	return (
		fAlpha * fAlpha * fAlpha * a * a / 6 + fAlpha * fAlpha * a * b / 2 + fAlpha * b * b / 2);
}
...
IC float CLevelGraph::compute_square(float fAngle, float fAngleOfView,
	const CLevelGraph::CVertex *vertex) const
{
	return (compute_square(fAngle, fAngleOfView, NORMALIZE_NODE_COVER(vertex, 0),
		NORMALIZE_NODE_COVER(vertex, 1), NORMALIZE_NODE_COVER(vertex, 2),
...
{
	return (
		cover_in_direction(angle, NORMALIZE_NODE_COVER(vertex, 0), NORMALIZE_NODE_COVER(vertex, 1),
...
IC bool CLevelGraph::check_vertex_in_direction(u32 start_vertex_id, const Fvector &start_position,
	u32 finish_vertex_id) const
{
	return (check_vertex_in_direction(start_vertex_id,
		Fvector2().set(start_position.x, start_position.z), finish_vertex_id));
}
