...
xr_string to_string(luabind::object const &o)
{
	...
	if (o.type() == LUA_TSTRING)
		return object_cast<xr_string>(o);
...
void print_class(lua_State *L, luabind::detail::class_rep *crep)
{
	...
	{
#ifndef USE_NATIVE_LUA_STRINGS
		typedef std::map<const char *, luabind::detail::class_rep::callback, luabind::detail::ltstr>
			PROPERTIES;
#else
	...
	{
		const std::vector<luabind::detail::construct_rep::overload_t> &constructors =
			crep->constructors().overloads;
		std::vector<luabind::detail::construct_rep::overload_t>::const_iterator I =
			constructors.begin();
		std::vector<luabind::detail::construct_rep::overload_t>::const_iterator E =
			constructors.end();
		for (; I != E; ++I)
		{
			xr_string S;
