...
struct SFillPropData
{
	...
	u32 counter;
	SFillPropData() { counter = 0; }
	~SFillPropData() { VERIFY(0 == counter); }
	void load()
	{
		// create ini
...
#ifdef XRSE_FACTORY_EXPORTS
	RListValue *value = PHelper().CreateRList(items,
		PrepareKey(pref, *base()->s_name, "npc profile"), &m_sCharacterProfile,
...
void CSE_ALifeGraphPoint::FillProps(LPCSTR pref, PropItemVec &items)
{
	...
	PHelper().CreateRList(items, PrepareKey(pref, *s_name, "Connection\\Level name"),
		&m_caConnectionLevelName, &*fp_data.level_ids.begin(), fp_data.level_ids.size());
...
void CSE_ALifeObject::FillProps(LPCSTR pref, PropItemVec &items)
{
	...
	PHelper().CreateRToken32(items, PrepareKey(pref, *s_name, "ALife\\Spawn Story ID"),
		&m_spawn_story_id, &*fp_data.spawn_story_names.begin(), fp_data.spawn_story_names.size());
#endif
}
...
u32 CSE_ALifeObject::ef_equipment_type() const
{
	...
	R_ASSERT3(false,
		"Invalid alife equipment type request, virtual function is not properly overloaded!", temp);
	return (u32(-1));
	//	return		(6);
}
...
u32 CSE_ALifeObject::ef_detector_type() const
{
	...
	CLSID2TEXT(m_tClassID, temp);
	R_ASSERT3(false,
		"Invalid alife detector type request, virtual function is not properly overloaded!", temp);
	return (u32(-1));
}
...
{
	return (
		!!m_flags.is(flInteractive) && !!m_flags.is(flVisibleForAI) && !!m_flags.is(flUsefulForAI));
}
...
xr_token po_types[] = {{"Box", epotBox}, {"Fixed chain", epotFixedChain},
	{"Free chain", epotFreeChain}, {"Skeleton", epotSkeleton}, {0, 0}};
...
void CSE_ALifeObjectHangingLamp::FillProps(LPCSTR pref, PropItemVec &values)
{
	...
	if (flags.is(flPointAmbient))
	{
		...
		PHelper().CreateChoose(values, PrepareKey(pref, *s_name, "Light\\Ambient\\Bone"),
			&light_ambient_bone, smSkeletonBones, 0, (void *)visual()->get_visual());
	}
...
{
	return (!object || (object->m_bDirectControl &&
...
u32 CSE_ALifeSchedulable::ef_creature_type() const
{
	...
	R_ASSERT3(false,
		"Invalid alife creature type request, virtual function is not properly overloaded!", temp);
	return (u32(-1));
}
...
u32 CSE_ALifeSchedulable::ef_anomaly_type() const
{
	...
	CLSID2TEXT(base()->m_tClassID, temp);
	R_ASSERT3(false,
		"Invalid alife anomaly type request, virtual function is not properly overloaded!", temp);
	return (u32(-1));
}
...
u32 CSE_ALifeSchedulable::ef_weapon_type() const
{
	...
	CLSID2TEXT(base()->m_tClassID, temp);
	R_ASSERT3(false,
		"Invalid alife weapon type request, virtual function is not properly overloaded!", temp);
	return (u32(-1));
}
