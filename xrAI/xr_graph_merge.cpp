...
public:
	...
	CLevelGameGraph(
#ifdef PRIQUEL
		LPCSTR graph_file_name, LPCSTR raw_cross_table_file_name,
#endif // PRIQUEL
		CGameGraph::SLevel *tLevel, LPCSTR S, u32 dwOffset, u32 dwLevelID, CInifile *Ini)
	{
		m_tLevel = *tLevel;
		m_dwOffset = dwOffset;
		...
		VERIFY(
			l_tpAI_Map->header().vertex_count() == l_tpCrossTable->header().level_vertex_count());
		...
		for (; I != E; I++)
		{
			...
			Memory.mem_copy((*I).tVertexTypes, m_tpGraph->vertex(int(I - B))->vertex_type(),
			...
			for (; i != e; ++i)
			{
				...
				edge = *i;
				VERIFY(
					(edge.vertex_id() + dwOffset) < (u32(1) << (8 * sizeof(GameGraph::_GRAPH_ID))));
	...
	void vfAddEdge(u32 dwVertexNumber, CGameGraph::CEdge &tGraphEdge)
	{
		...
		m_tpVertices[dwVertexNumber].tpaEdges =
			(CGameGraph::CEdge *)xr_realloc(m_tpVertices[dwVertexNumber].tpaEdges,
	...
	void vfGenerateDeathPoints(int iGraphIndex, CGameLevelCrossTable *tpCrossTable,
	...
	xr_vector<LPCSTR> *needed_levels)
{
	...
	for (u32 k = 0; Ini->r_line("levels", k, &_N, &V); k++)
	{
		...
		if (!FS.exist(file_name))
		{
			...
			if (!FS.exist(file_name))
			{
				Msg("! There is no graph for the level %s! (level is not included into the game "
					"graph)",
					S);
				continue;
			}
...

		{
			...
			if (header.version() != XRAI_CURRENT_VERSION)
			{
				Msg("! Graph for the level %s is incompatible (version mismatch)! (level is not "
					"included into the game graph)",
					S);
				continue;
			}
		...
		if (!FS.exist(file_name))
		{
			Msg("! There is no cross table for the level %s! (level is not included into the game "
				"graph)",
				S);
			continue;
		}

		{
			...
			{
				FS.r_close(reader);
				Msg("! Cross table for the level %s is corrupted! (level is not included into the "
					"game graph)",
					S);
				continue;
			}
			...
			if (header.version() != XRAI_CURRENT_VERSION)
			{
				Msg("! Cross table for the level %s is incompatible (version mismatch)! (level is "
					"not included into the game graph)",
					S);
				continue;
			}
		}
