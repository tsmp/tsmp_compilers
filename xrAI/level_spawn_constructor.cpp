...
#include "ServerEntities\xrmessages.h"
...
{
	for (auto &graphPoint : m_graph_points)
		Factory::DestroyEntity((CSE_Abstract *&)graphPoint);
...
CSE_Abstract *CLevelSpawnConstructor::create_object(IReader *chunk)
{
	...
	netPacket.B.count = chunk->length();
	chunk->r(netPacket.B.data, netPacket.B.count);

	// we do not need to close chunk since we iterate on them
	// chunk->close();

	u16 packetType;
	netPacket.r_begin(packetType);
	R_ASSERT2(M_SPAWN == packetType, "packetType doesn't match to the spawn-point ID!");

	string64 sectionName;
	netPacket.r_stringZ(sectionName);

	if (CSE_Abstract *abstr = Factory::CreateEntity(sectionName))
	{
		abstr->Spawn_Read(netPacket);
		return abstr;
	}

	string256 temp;
	sprintf(temp, "Can't create entity '%s' !\n", sectionName);
	R_ASSERT2(false, temp);
	return nullptr;
}
...
void CLevelSpawnConstructor::load_objects()
{
	...
	for (; chunk; chunk = level_spawn->open_chunk_iterator(id, chunk))
	{
		...
		{
			Factory::DestroyEntity(abstract);
			continue;
		}
	...
	for (int i = 0; i < (int)m_spawns.size(); i++)
	{
			...
			!level_graph().inside(level_graph().vertex(m_spawns[i]->m_tNodeID), position))
		{
			...
			clMsg("[%s][%s][%s] : position changed from [%f][%f][%f] -> [%f][%f][%f]",
				*m_level.name(), *m_spawns[i]->s_name, m_spawns[i]->name_replace(), VPUSH(position),
				VPUSH(new_position));
		...
		if (game_graph().vertex(dwBest)->level_id() != m_level.id())
		{
			...
			char *S = S1;
			S += sprintf(S,
				"Corresponding graph vertex %s for the spawn point is located on the ANOTHER "
				"level\n",
...
void CLevelSpawnConstructor::generate_artefact_spawn_positions()
{
	...
	for (; I != E; I++)
	{
		...
		if (zone->m_artefact_spawn_count >= l_tpaStack.size())
		{
			...
			{
				Msg("! CANNOT GENERATE ARTEFACT SPAWN POSITIONS FOR ZONE [%s] ON LEVEL [%s]",
					zone->name_replace(), *level().name());
				Msg("! ZONE [%s] ON LEVEL [%s] IS REMOVED BY AI COMPILER", zone->name_replace(),
					*level().name());
				R_ASSERT3(zone->m_story_id == INVALID_STORY_ID, "Cannot remove story object",
					zone->name_replace());
				R_ASSERT3(!zone->m_spawn_control, "Cannot remove spawn control object",
					zone->name_replace());
			...
			std::shuffle(l_tpaStack.begin(), l_tpaStack.end(), std::default_random_engine{});
	...
	for (u32 i = 0, n = (u32)level_changers().size(); i < n; ++i)
	{
		...
		for (; I != E; ++I)
			if (!xr_strcmp(*level_changers()[i]->m_caLevelPointToChange, (*I)->name_replace()))
			{
				...
				R_ASSERT3(ok,
					"Cannot find a correspndance between graph and graph points from level editor! "
					"Rebuild graph for the level ",
		...
		if (!found)
		{
			clMsg("Graph point %s not found (level changer %s)",
				*level_changers()[i]->m_caLevelPointToChange, level_changers()[i]->name_replace());
			VERIFY(false);
		}
	}
}
