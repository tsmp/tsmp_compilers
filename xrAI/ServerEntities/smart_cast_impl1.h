...
#include "Object\object_type_traits.h"
...
namespace SmartDynamicCast
{
	...
	struct has_conversion
	{
		...
		struct search_conversion
		{
			...
			{
				value =
					selector<object_type_traits::is_same<typename T::Head, Source>::value>::value
			};
		};
		...
		struct CMatchHelper2
		{
			...
			struct CMatchHelper3
			{
				...
				struct selector
				{
					...
					typedef typename _selector<object_type_traits::is_same<Head,
	...
	struct conversion_sequence
	{
		...
		struct list_iterator
		{
			...
			struct helper
			{
				typedef typename conversion_sequence<Target, T, length,
					use_heritage /**,new_visited/**/>::result search_result;
				...
				typedef
					typename selector<is_type<Loki::NullType, search_result>::value>::result result;
			};
			...
			{
				typedef
					typename helper<typename Head::Head, max_length, false>::result helper_result;
				...
				typedef typename _selector<!is_type<Loki::NullType, helper_result>::value>::result
					result;
			};
			...
			struct selector<false>
			{
				...
				struct _selector
				{
					...
					typedef
						typename _selector2<!is_type<Loki::NullType, helper_result>::value>::result
							result;
				};
				...
				typedef
					typename _selector<has_conversion<Source, typename Head::Head>::value>::result
						result;
			};
...
IC T1 smart_cast(T2 *p)
{
	...
	return (
		SmartDynamicCast::CHelper2<T2>::smart_cast<object_type_traits::remove_pointer<T1>::type>(
			p));
}
...
#endif
#ifdef SMART_CAST_STATS_ALL
	add_smart_cast_stats_all(typeid(T2 *).name(),
