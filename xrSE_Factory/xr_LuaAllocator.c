	...
	for (i = 1; i < NBINS; ++i)
	{
		...
		for (p = last(b); p != b; p = p->bk)
		{
			...
			if (i >= 2)
			{
				...
				{
					assert(
						p->bk == b || (CHUNK_SIZE_T)chunksize(p->bk) >= (CHUNK_SIZE_T)chunksize(p));
				}
			}
...
#endif
{
	...
	if (!chunk_is_mmapped(oldp))
	{
		...
		else
		{
			...
			if (next == av->top &&
				(CHUNK_SIZE_T)(newsize = oldsize + chunksize(next)) >= (CHUNK_SIZE_T)(nb + MINSIZE))
			{
...
#endif
{
  ...
  */
    ...
    */
		...
		brk = (char *)mem2chunk(
			(PTR_UINT)(((PTR_UINT)(m + alignment - 1)) & -((signed long)alignment)));
...
static void *sbrk(long size)
{
	...
	if (size >= 0)
	{
		...
		long to_commit =
			(char *)g_last->top_allocated + allocate_size - (char *)g_last->top_committed;
		...
		if (to_commit > 0)
		{
			...
			long to_reserve =
				(char *)g_last->top_committed + commit_size - (char *)g_last->top_reserved;
				...
				if (remaining_commit_size > 0)
				{
					...
					assert(0 < remaining_commit_size && remaining_commit_size % g_pagesize == 0);
					{
						...
						void *base_committed = VirtualAlloc(g_last->top_committed,
							remaining_commit_size, MEM_COMMIT, PAGE_READWRITE);
			...
			assert(0 < commit_size && commit_size % g_pagesize == 0);
			{
				/* Commit this */
				void *base_committed =
					VirtualAlloc(g_last->top_committed, commit_size, MEM_COMMIT, PAGE_READWRITE);
				/* Check returned pointer for consistency */
				if (base_committed != g_last->top_committed)
					goto sbrk_exit;
				/* Assert postconditions */
	...
	else if (size < 0)
	{
		...
		}
		{
			...
			long to_decommit =
				(char *)g_last->top_committed - ((char *)g_last->top_allocated - deallocate_size);
...
static int cpuinfo(int whole, CHUNK_SIZE_T *kernel, CHUNK_SIZE_T *user)
{
	...
	{
		__int64 creation64, exit64, kernel64, user64;
		int rc = GetThreadTimes(GetCurrentThread(), (FILETIME *)&creation64, (FILETIME *)&exit64,
			(FILETIME *)&kernel64, (FILETIME *)&user64);
		if (!rc)
		{
			*kernel = 0;
